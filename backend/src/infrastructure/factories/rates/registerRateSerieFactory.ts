...
export function registerRateSerieFactory(prismaService: PrismaService) {
  const rateRepository = new RateRepository(prismaService);
  const registerRateSerieUseCase = new RegisterRateMovieUseCase(rateRepository);
  const controller = new RegisterRateMovieController(registerRateSerieUseCase);
  return controller;
}
